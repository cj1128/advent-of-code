...
def new_node(name, weight=None, parent=None):
  return {
    "name": name,
    "weight": weight,
    "parent": parent,
...
def build_tree(input):
  ...
  for line in input.strip().split("\n"):
    name, weight, children_names = tree_re.match(line).groups()
    weight = int(weight)
    ...
    if children_names:
      ...
      for child_name in children_names.split(", "):
        if nodes.get(child_name):
          ...
          child = new_node(child_name, parent=node)
          nodes[child_name] = child
        children.append(child)
      node["children"] = children
      if root is None or root in children:
...
def solve_part2(input):
  ...
  cur = cur["children"][0]
  print_weight(cur)
  # we know cur is wrong
  print(cur["weight"])
  # correct weight should be: 1513 - 8 = 1505
