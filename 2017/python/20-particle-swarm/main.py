...
particle_re = re.compile(r"^p=<(.+)>, v=<(.+)>, a=<(.+)>$")

Particle = namedtuple("Particle", ["id", "px", "py", "pz", "vx", "vy", "vz", "ax", "ay", "az"])

def parse_particle(id, str):
  p, v, a, = particle_re.match(str).groups()
  px, py, pz = [int(n) for n in p.strip().split(",")]
  vx, vy, vz = [int(n) for n in v.strip().split(",")]
  ax, ay, az = [int(n) for n in a.strip().split(",")]
  return Particle(id, px, py, pz, vx, vy, vz, ax, ay, az)
...
def tick(p):
  ...
  py = p.py + vy
  pz = p.pz + vz
  return p._replace(vx=vx, vy=vy, vz=vz, px=px, py=py, pz=pz)

# 其实这道题用物理学的方法想一想就知道，从长远来看，离0点最近的一定是加速度最小的
# 凡是加速度比它大的，足够长的时间以后，一定离的比它远
def solve_part1(input):
  particles = [parse_particle(id, str) for id, str in enumerate(input.strip().split("\n"))]
  closest_particle = min(particles, key=lambda p: abs(p.ax) + abs(p.ay) + abs(p.az))
...
def solve_part2(input):
  particles = [parse_particle(id, str) for id, str in enumerate(input.strip().split("\n"))]
  for i in range(1000):
    particles = [tick(p) for p in remove_collision(particles)]
