...
def parse(line):
  g = instruction_re.match(line).groups()
  return {
    "reg": g[0],
    "op": g[1],
    "value": int(g[2]),
...
def match_condition(reg, condition, value):
  if condition == ">":
    return reg > value
  if condition == ">=":
    return reg >= value
...
def solve_part1(input):
  ...
  for line in input.strip().split("\n"):
    res = parse(line)
    if match_condition(regs[res["creg"]], res["con"], res["cvalue"]):
      if res["op"] == "inc":
        regs[res["reg"]] +=  res["value"]
      else:
        regs[res["reg"]] -=  res["value"]
...
def solve_part2(input):
  ...
  for line in input.strip().split("\n"):
    ...
    if match_condition(regs[res["creg"]], res["con"], res["cvalue"]):
      if res["op"] == "inc":
        new_value = regs[res["reg"]] + res["value"]
      else:
        new_value = regs[res["reg"]] - res["value"]
      regs[res["reg"]] = new_value
